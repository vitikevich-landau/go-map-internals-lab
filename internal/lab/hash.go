...
func hashInt(key MapKey, seed HashSeed) FullHash {
	...
	// Раунды исключающего ИЛИ со сдвигом и умножения распространяют небольшое
	// изменение ключа на весь 64-битный результат. Эти числа — константы
	// перемешивания, а не служебные данные map.
	x ^= x >> 30
...
// h1 возвращает часть хеша, которая используется для маршрутизации и
// последовательности пробирования.
//
...
// h2 возвращает семибитный отпечаток, который хранится в управляющем байте
// занятого слота.
//
